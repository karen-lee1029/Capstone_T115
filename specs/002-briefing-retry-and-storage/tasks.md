...
- [ ] T001 From `student_attrition_risk_app/`, run `uv sync --dev`, then `uv run ruff check .` and `uv run pytest`; record that the Feature-001 suite is green before any Feature-002 change.
...
- [ ] T021 [P] Update `student_attrition_risk_app/README.md` to document the single-retry workflow (`SingleRetryWorkflow`, exactly one retry, `attempt_count = 2`), the governed `VolumeBriefingStore` (append-only, one JSON file per validated briefing, most-recent retrieval), and the `BRIEFING_VOLUME` setting (blank ⇒ in-memory store).
- [ ] T022 From `student_attrition_risk_app/`, run the full merge gate: `uv run ruff check .` and `uv run pytest` — all Feature-001 and Feature-002 suites green.
- [ ] T023 Walk the offline sections of `specs/002-briefing-retry-and-storage/quickstart.md` (retry-workflow table, governed-store table, store-selection) and confirm each stated expectation holds.
